...
public class WechatServiceImpl implements WechatService {
    ...
    public String getWechatAuthorizeUrl(String app, HttpServletRequest request, String toUrl)
        ...
        stringRedisTemplate.opsForValue().set(STATE_WECHAT, state, 1, TimeUnit.HOURS);
    ...
    public WechatUserInfo getWechatUserInfo(String app, HttpServletRequest request, String code, String state) {
        ...
        //微信登录之后自动新增用户，用户名密码均为openid
        UserCredential userCredential = userCredentialsDao.findByUsername(wechatUserInfo.getOpenid());
        if (userCredential == null) {
            AppUser user = new AppUser();
            user.setUsername(wechatUserInfo.getOpenid());
            user.setPassword(passwordEncoder.encode(wechatUserInfo.getOpenid()));
            appUserDao.save(user);
            userCredentialsDao
                    .save(new UserCredential(user.getUsername(), CredentialType.WECHAT_OPENID.name(), user.getId()));
            log.info("微信登录添加用户：{}", user);
        }
    ...
    private void checkStateLegal(String state, HttpServletRequest request) {
        ...
        String sessionState = stringRedisTemplate.opsForValue().get(STATE_WECHAT);
        ...
        stringRedisTemplate.delete(STATE_WECHAT);
    }
    ...
    public String cacheWechatUserInfo(WechatUserInfo wechatUserInfo) {
